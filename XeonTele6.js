...
module.exports = XeonBotInc = async (XeonBotInc, bot) => {
    ...
    try {
        ...
        const username = XeonBotInc.message.from.username ? XeonBotInc.message.from.username : "darkempdev";
...
break;
    ...
    /*if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return XeonBotInc.telegram.sendMessage(
            ...
            `📩 *Send proof to the owner @darkempdev*`,
            {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "📢 Telegram Channel", url: "https://t.me/codexemp" }],
                        [{ text: "▶️ YouTube Channel", url: "https://youtube.com/@unveiledhacking" }],
                        [{ text: "📱 WhatsApp Channel", url: "https://whatsapp.com/channel/0029Vb70IdY60eBmvtGRT00R" }]
                    ]
                },
    ...
    return XeonBotInc.telegram.sendMessage(
        XeonBotInc.chat.id, 
        `🚫 *You are not authorized to use this command\\.*\n\n` +
        `📩 *Please contact the developer to buy:* @darkempdev\n\n` +
        `💰 *Price/Harga:*\n` +
        `✅ *Access permanent:* 15$\n` +
        `✅ *Resell permanent:* 25$\n` +
        `✅ *Script no enc 100%:* 100$`,
        {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "📢 Telegram Group", url: "https://t.me/darkempirechatroom" }],
                    [{ text: "▶️ YouTube Channel", url: "https://youtube.com/@unveiledhaking" }],
                    [{ text: "📱 WhatsApp Channel", url: "https://whatsapp.com/channel/0029Vb70IdY60eBmvtGRT00R" }]
                ]
            },
            parse_mode: "MarkdownV2"
...
case 'runtime':{
    XeonBotInc.deleteMessage().catch(() => {});
      reply(`Alastor Bug Bot is Online ${runtime(process.uptime())}`)
    }
  break
case 'menu': case 'back!':
