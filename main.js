...
require('./config');
   // ==================== UPDATED IMPORTS - USE CORRECT BAILEYS ====================
   const { default: makeWASocket, DisconnectReason, makeInMemoryStore, jidDecode, Browsers, proto, getContentType, useMultiFileAuthState, fetchLatestBaileysVersion, downloadContentFromMessage, areJidsSameUser, jidNormalizedUser } = require("@whiskeysockets/baileys")
   ...
   const { state, saveCreds } = await useMultiFileAuthState("./session")  // FIXED: Added ./ prefix
   ...
   XeonBotInc.ev.on('connection.update', async (update) => {
       ...
       if (connection === 'close') {
           ...
           await sleep(1999);
   // REMOVED PROBLEMATIC AUTO-PAIRING CODE
   // This was causing errors and not needed for standard pairing
   }
   });
